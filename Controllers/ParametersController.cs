...
namespace AirMonit_Service.Controllers
{
    public class ParametersController : ApiController
    {
            ...
            using (var conn = new SqlConnection(connectionString))
            {
                ...
                using (SqlDataReader objReader = cmd.ExecuteReader())
                {
                    ...
                    if (numValues != 0)
                    {
                        ...
                        listaHourly.Add(new Hour { avg = avgHour, hour = hour, max = max, min = min });
                    }
                }
                conn.Close();
        ...
        public IHttpActionResult GetHourlySummarizedInformationAllCities(string parameter, string date1,string date2, string city)
        {
            string str = "";
            SqlCommand cmd;
            List<Hour> listaHourly = new List<Hour>();
